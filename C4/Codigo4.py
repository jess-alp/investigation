...
def g(x1, x2):
    return 2 * 418.9829 - x1 * math.sin(math.sqrt(abs(x1))) - x2 * math.sin(math.sqrt(abs(x2)))

def fitness(ind):
    return 1 / (1 + g(ind[0], ind[1]))

def generar_poblacion():
    return [[random.uniform(*rango), random.uniform(*rango)] for _ in range(poblacion_tam)]
...
def elitismo(poblacion, nueva_pob):
    ...
    nueva_pob[random.randint(0, poblacion_tam-1)] = mejor
    return nueva_pob

poblacion = generar_poblacion()

for gen in range(generaciones):
    mejor_ind = min(poblacion, key=lambda ind: g(ind[0], ind[1]))
    mejor_val = g(mejor_ind[0], mejor_ind[1])

    if mejor_val < 1e-6:
        contador_soluciones += 1

    nueva_pob = []
    while len(nueva_pob) < poblacion_tam:
...
mejor = min(poblacion, key=lambda ind: g(ind[0], ind[1]))
print("Mejor solución encontrada:", mejor)
print("Valor de la función:", g(mejor[0], mejor[1]))
